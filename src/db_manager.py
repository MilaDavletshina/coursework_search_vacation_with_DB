...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self):
        ...
        self.cur.execute(
            f"SELECT employers.company_name, COUNT(vacancies.vacancy_id) AS vacancy_count"
            f"FROM employers LEFT JOIN vacancies ON employers.company_id = vacancies.company_id"
            f"GROUP BY employers.company_id ORDER BY employers.company_name"
        )
    ...
    def get_all_vacancies(self):
        ...
        self.cur.execute(
            ...
            f"from vacancies left join employers using (company_id)"
            f"order by vacancies.vacancy_name desc"
        )
        return self.cur.fetchall()
    ...
    def get_avg_salary(self):
        ...
        self.cur.execute(
            f"select AVG(salary_from) as avg_salary from vacancies WHERE salary_from IS NOT NULL"
        )
        return self.cur.fetchall()
    ...
    def get_vacancies_with_higher_salary(self):
        ...
        self.cur.execute(
            f"select vacancies.vacancy_name, vacancies.salary_from from vacancies"
            f"where vacancies.salary_from > (select AVG(salary_from) from vacancies)"
        )
        return self.cur.fetchall()
